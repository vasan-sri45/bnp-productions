"use client";

import React from "react";

const Breadcrumb = ({ pageName, pageDescription }) => {
  const handleHomeClick = (e) => {
    e.preventDefault();
    // Add your navigation logic here (router.push, etc.)
    window.scrollTo({ top: 0, behavior: "smooth" });
  };

  return (
    <div className="dark:bg-darkmode relative z-10 overflow-hidden pb-[60px] pt-[120px] md:pt-[130px] lg:pt-[160px]">
      <div className="from-stroke/0 via-stroke to-stroke/0 dark:via-dark-3 absolute bottom-0 left-0 h-px w-full bg-linear-to-r" />

      <div className="container mx-auto">
        <div className="-mx-4 flex flex-wrap items-center">
          <div className="w-full px-4">
            <div className="text-center">
              <h1 className="mb-4 text-3xl font-bold text-black sm:text-4xl md:text-[40px] md:leading-[1.2] dark:text-white">
                {pageName}
              </h1>

              <p className="mb-5 text-base text-black dark:text-black-6">
                {pageDescription}
              </p>

              <ul className="flex items-center justify-center gap-[10px]">
                <li>
                  <a
                    href="/"
                    onClick={handleHomeClick}
                    className="flex items-center gap-[10px] text-base font-medium text-black hover:underline dark:text-white dark:text-opacity-50"
                  >
                    Home
                  </a>
                </li>

                <li>
                  <p className="flex items-center gap-[10px] text-base font-medium text-body-color">
                    <span className="text-body-color dark:text-white dark:text-opacity-50">
                      /
                    </span>
                    {pageName}
                  </p>
                </li>
              </ul>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Breadcrumb;
